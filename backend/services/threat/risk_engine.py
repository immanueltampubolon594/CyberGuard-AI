...
def calculate_risk(results: dict, url: str = "") -> dict:
    ...
    if "phishtank" in results:
        ...
        if pt.get("verified"):
            ...
            total_score += 50
            reasons.append(f"⚠️  PhishTank: URL ada di database phishing (belum verified)")
    ...
    if "abuseipdb" in results:
        ...
        if abuseipdb.get("malicious"):
            total_score += 20
            reasons.append("🛡️ AbuseIPDB: IP memiliki reputasi buruk")
    ...
    if "ssl" in results:
        ...
        # ✅ HANYA tambah score jika ada masalah
        if ssl.get("is_self_signed"):
            total_score += 60
            reasons.append("🚫 SSL Certificate self-signed (berbahaya)")
        elif ssl.get("is_trusted_issuer"):
            # ✅ Trusted issuer = 0 score (AMAN!)
            # Jangan tambah score
            pass
        elif ssl.get("is_lets_encrypt"):
            total_score += 5
            reasons.append("⚠️  SSL gratis Let's Encrypt")
        elif ssl_score > 0:
    ...
    if "url_expansion" in results:
        ...
        if expansion.get("is_shortened"):
            ...
            if hops > 2:
                total_score += 20
                reasons.append(f"⚠️  Banyak redirect ({hops} hops) - teknik cloaking")
            elif hops > 0:
                total_score += 10
