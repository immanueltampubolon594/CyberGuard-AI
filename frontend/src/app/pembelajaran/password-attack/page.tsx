  ...
  return (
    <div className="p-6 rounded-2xl border border-amber-200 dark:border-amber-500/15 bg-amber-50 dark:bg-amber-500/[0.04]">
      <div className="flex items-center gap-2 mb-3">
        ...
        <p className="text-[10px] font-black text-amber-500 uppercase tracking-widest">Kasus Nyata</p>
      </div>
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#020617] text-slate-600 dark:text-slate-300 font-sans transition-colors duration-500">
      <div className="flex">
        ...
        <aside className="fixed left-0 top-20 w-72 h-[calc(100vh-5rem)] border-r border-slate-200 dark:border-slate-800/60 px-6 py-8 hidden lg:flex flex-col bg-white dark:bg-[#020617] z-10 overflow-y-auto transition-all">
          ...
          {/* Breadcrumb */}
          <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-500 mb-6 uppercase tracking-widest">
            <Link href="/pembelajaran" className="hover:text-slate-400 transition-colors">Pembelajaran</Link>
            <ChevronRight size={10} />
            <span className="text-blue-500">Password Attack</span>
          </div>
          ...
          <nav className="space-y-1.5 flex-1">
            {tiersNav.map((tier: any) => {
              ...
              return (
                ...
                >
                  <div className={`mt-0.5 shrink-0 ${isActive ? "text-blue-400" : "text-slate-400"}`}>
                    {iconMap[tier.icon]}
                  </div>
                  <div>
                    <p className={`text-[9px] font-black uppercase tracking-widest mb-0.5 ${isActive ? "text-blue-500" : "text-slate-400"}`}>
          ...
          <div className="mt-6 pt-5 border-t border-slate-200 dark:border-slate-800/60">
            <div className="flex items-center justify-between mb-2">
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-wider">Progress</p>
              <p className="text-[11px] font-bold text-slate-400">{activeTier}/{totalTiers - 1}</p>
            </div>
        ...
        <main className="flex-1 lg:ml-72 min-h-screen bg-slate-50 dark:bg-[#020617]">
          ...
          <div className="px-8 lg:px-12 xl:px-16 py-12 w-full max-w-none">
            ...
            {/* Page title */}
            <div className="mb-10">
              <p className="text-blue-500 text-[11px] font-black tracking-[6px] uppercase mb-3">Password Attack</p>
              ...
              {t?.definition && (
                <div className="space-y-6">
                  {t.definition.split("\n\n").map((paragraph: string, index: number) => (
                    <p key={index} className="text-lg leading-9 text-slate-700 dark:text-slate-300">
                      {paragraph}
                    </p>
                  ))}
                </div>
              ...
              {t?.sections?.map((section: any, index: number) => (
                <div key={index} className="space-y-6">
                  ...
                  {section.content.map((paragraph: string, pIndex: number) => (
                    <p key={pIndex} className="text-lg leading-9 text-slate-700 dark:text-slate-300">
                      {paragraph}
                    </p>
                  ))}
                </div>
            ...
            <div className="mt-16 pt-8 border-t border-slate-200 dark:border-slate-800/60 flex items-center justify-between">
              ...
              {activeTier < totalTiers - 1 ? (
                ...
                <Link
                  href="/pembelajaran"
                  className="flex items-center gap-3 bg-slate-900 dark:bg-white hover:bg-slate-700 dark:hover:bg-slate-100 text-white dark:text-[#020617] px-8 py-3.5 rounded-xl text-[12px] font-black uppercase tracking-wider transition-all"
                >
